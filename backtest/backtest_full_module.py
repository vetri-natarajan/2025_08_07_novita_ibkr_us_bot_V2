...
async def main():
    ...
    fetcher = HistoricalDataFetcher(ib)

    watchlist = ['AAPL', 'MSFT']
    account_value = 100000
    cfg = {
        'TRADING': {'units': 5},
        'EXIT': {'default_method': 'fixed', 'fixed_sl_pct': 0.01, 'fixed_tp_pct': 0.02},
        'STRATEGY': {'dummy_signal': True}
    }
    # Example timeframes: you can use ['1 month', '1 week', '1 day'] or ['30 min', '5 min', '1 min']
    timeframes = ['1 day', '5 min', '1 min']

    backtester = BacktestEngine(watchlist, account_value, cfg, fetcher)
    await backtester.run_backtest(timeframes, duration_value=2, duration_unit='weeks')
