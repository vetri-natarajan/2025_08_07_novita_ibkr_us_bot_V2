...
class BacktestEngine:
    ...
    def __init__(self, ib, ALWAYS_TFS, watchlist, account_value, data_fetcher, config_dict, premarket, watchlist_main_settings, logger, commission=0.0005):
        ...
        self.account_value = float(account_value)  # cash-like account balance (realized only) 
        self.positions = {}
        self.trades = []
        ...
        self.percent_of_account_value = float(self.config_dict['percent_of_account_value'])  # e.g., 0.1 for 10% [attached_file:1]
        ...
        self.total_trades = 0
        self.net_pnl = 0
        self.floating_equity = float(account_value)  # initialize equal to cash [attached_file:1]
    ...
    # NEW: compute unrealized PnL across open positions and update floating equity
    def compute_unrealized_and_equity(self, price_lookup: dict) -> tuple:
        """
        price_lookup: dict[symbol_combined] -> last_price (float)
        Returns: (total_unrealized_pnl, floating_equity)
        """
        total_unreal = 0.0
        for sym, pos in self.positions.items():
            ...
            last = float(price_lookup.get(sym, entry))
            if side == 'BUY':
                unreal = (last - entry) * qty
            else:
                unreal = (entry - last) * qty
            total_unreal += unreal
        floating_equity = self.account_value + total_unreal
        self.floating_equity = floating_equity
    ...
    def enter_trade(self, symbol_combined, symbol, price, qty, sl_price, tp_price, signal, entry_time, side="BUY"):
        ...
        # If insufficient cash/equity for entry, skip (cash check; floating equity used for sizing)
    ...
    async def run_backtest(
        ...
        self.floating_equity = float(self.account_value)
        ...
        # 2) Pre-compute unique trading days
        ...
        for current_day_local in tqdm(global_days_local, desc="Days", position=0, leave=True):
            ...
            for current_time_utc in tqdm(day_times, desc=f"Bars {current_day_local.date()}", position=1, leave=False):
                ...
                price_lookup = {}
                for sym_c, meta in universe.items():
                    df_LTF_all = meta['df_LTF']
                    if current_time_utc < df_LTF_all.index[0]:
                        continue
                    df_LTF_slice_tmp = df_LTF_all.loc[:current_time_local]
                    if not df_LTF_slice_tmp.empty:
                        price_lookup[sym_c] = float(df_LTF_slice_tmp['close'].iloc[-1])

                # Update unrealized and floating equity
                unreal, feq = self.compute_unrealized_and_equity(price_lookup)
                self.logger.info(f"🧮 Floating equity: {feq:.2f} | Cash: {self.account_value:.2f} | Unrealized: {unreal:.2f}")
    ...
    def write_trades_to_file(self, backtest_dir, file):
        ...
        # Optional: prepend a summary header file for starting/ending equity
