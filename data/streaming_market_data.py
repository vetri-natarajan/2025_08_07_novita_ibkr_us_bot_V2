...
class StreamingData:
    ...
    async def seed_historical(self, contract: Contract, timeframe: str, max_tf: str, max_look_back: int) -> Optional[pd.DataFrame]:
        ...
        if max_tf.lower() == "1 week":           
            ...
            raise ValueError("⚠️ Invalid max timeframe. Should be '1 week' or '30 mins'.")
         
        self.logger.info(f"⌛️ Seeding  bars of {timeframe} ({duration_str}) for {contract.symbol}...")
        
        try:
            # Run synchronous call in executor to avoid blocking asyncio loop
