...
class BarAggregator:
    ...
    def add_bar(self, bar: AggregatedBar) -> Optional[AggregatedBar]:
        if bar is None:
            self.logger.error("Attempted to add None to bars deque!")
            return None
    ...
    def partial_aggregate_bars(self) -> Optional[AggregatedBar]:
        ...
        if not filtered_bars:
            return None
        open_ = filtered_bars[0].open_
        high = max(b.high for b in filtered_bars)
        low = min(b.low for b in filtered_bars)
        close = filtered_bars[-1].close
        volume = sum(b.volume for b in filtered_bars)
    ...
    def _normalize_time(self, dt: datetime) -> datetime:
        ...
        if self.bars_to_aggregate < 1:
            ...
            new_hour = rounded_seconds // 3600
            new_minute = (rounded_seconds % 3600) // 60
            new_second = rounded_seconds % 60
            return dt.replace(hour=new_hour, minute=new_minute, second=new_second, microsecond=0)
        else:
